...
void selectionSort() {
    for (int i = 0; i < numSelecionados - 1; i++) {
        int menorIndice = i;
        
        for (int j = i + 1; j < numSelecionados; j++) {
            numeroComparacoes++;
            if (strcmp(jogosSelecionados[j].name, jogosSelecionados[menorIndice].name) < 0) {
                menorIndice = j;
            }
        }
        ...
        if (menorIndice != i) {
            Game temp;
            copyGame(&temp, &jogosSelecionados[i]);
            copyGame(&jogosSelecionados[i], &jogosSelecionados[menorIndice]);
            copyGame(&jogosSelecionados[menorIndice], &temp);
