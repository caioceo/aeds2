...
import Algoritimos.TADs.Arvores.AVL.No;
...
class ArvoreBinaria {
    ...
    private No inserir(Game game, No no) {
        if (no == null) {
            return new No(game);
        }
        ...
        if (cmp < 0) {
            no.esq = inserir(game, no.esq);
        } else if (cmp > 0) {
            no.dir = inserir(game, no.dir);
        }
    ...
    public void pesquisar(String nome) {
        ...
        resetComparacoes();
        boolean resultado = pesquisar(nome, raiz);
...
public class TP07Q01 {
    ...
    public static void main(String[] args) {
        ...
        try {
            FileWriter escritor = new FileWriter(matricula + "_arvoreBinaria.txt");
